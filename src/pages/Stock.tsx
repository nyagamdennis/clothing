...
import {
  ...
  selectAllStocks,
  updatingStock,
} from "../features/stock/stocksSlice"
...
const Stock = () => {
  ...
  const [submittingNewStock, setSubmittingNewStock] = useState(false)
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 bg-gray-100">
        ...
        <div className="mx-8 max-h-[550px] overflow-y-auto">
          <div className=" py-6 flex space-x-4"></div>
          ...
          <div className=" flex items-center w-1/2 relative  mt-4">
            ...
            <div
              ...
              className=" border rounded-lg px-2 bg-white cursor-pointer py-2 ms-5 flex items-center gap-2 text-orange-600"
            >
            ...
            <table className="min-w-full text-left text-gray-900 border-collapse  divide-y divide-gray-200">
              <thead className=" text-sm font-semibold">
                <tr className="text-gray-600">
                  ...
                  <th className="px-6 py-4">Total Size(metre)</th>
                ...
                {filteredMaterials.map(stock => (
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <MetricFormatter total={stock.total} />
                    </td>
                    <td className="px-6 py-4">
                      {CurrencyFormatter({
                        amount: stock.buying_price,
                        currencySymbol: "Ksh",
                        asString: true,
                      }) || ""}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex space-x-3 text-blue-600">
                        <button
                          onClick={() => handleClickOpenAddStock(stock)}
                          className=" bg-blue-500 text-white font-bold text-sm px-2 py-0.5 rounded-md"
                        >
                          Add
                        </button>
                        <button
                          onClick={() => handleClickOpenUpdate(stock)}
                          className="bg-green-500 text-white font-bold text-sm px-2 py-0.5 rounded-md"
                        >
                          Update
                        </button>
                        <button
                          onClick={() => handleClickOpen(stock)}
                          className="bg-red-500 text-white font-bold text-sm px-2 py-0.5 rounded-md"
                        >
                          Delete
                        </button>
                      </div>
          ...
          >
            ...
            <DialogActions>
              <button
                className="bg-green-500 px-2 py-0.5 text-white font-bold text-sm rounded-md"
                onClick={handleClickCloseAddStock}
              >
                Cancel
              </button>
              <button
                className="bg-red-500 px-2 py-0.5 text-white font-bold text-sm rounded-md"
                onClick={handleAddRolls}
                autoFocus
              >
                add
              </button>
          ...
          >
            ...
            <form onSubmit={handleAddStock}>
              ...
              <DialogActions className="flex gap-2">
                <button
                disabled = {submittingNewStock}
                className="  rounded-md bg-green-50 hover:bg-green-300 px-2 py-0.5">
                  Cancel
                </button>
                <button className=" flex items-center rounded-md bg-blue-50 hover:bg-blue-300 px-2 py-0.5">
                  {submittingNewStock ? (
                    <CircularProgress
                      color="secondary"
                      className=" !h-5 !w-5"
